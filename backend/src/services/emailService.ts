// backend/src/services/emailService.ts
import nodemailer, {
  type Transporter,
  type SendMailOptions,
  type SentMessageInfo,
} from "nodemailer";

/* ============================================================
 * TIPOS
 * ============================================================ */

interface TemplateInvitacionParams {
  nombreUsuario?: string;
  urlActivacion: string;
}

interface TemplateResetParams {
  nombreUsuario?: string;
  urlReset: string;
}

/* ============================================================
 * HELPERS
 * ============================================================ */

/**
 * Convierte strings tipo "true" / "false" a boolean.
 */
const parseBoolean = (value: unknown): boolean => {
  return String(value).toLowerCase() === "true";
};

/**
 * Escapa texto dinámico que se inyecta en HTML.
 */
const escaparHtml = (valor: unknown): string => {
  return String(valor ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
};

/**
 * Valida las variables mínimas SMTP.
 */
const validarConfiguracionSmtp = (): void => {
  const variablesObligatorias = [
    "SMTP_HOST",
    "SMTP_PORT",
    "SMTP_USER",
    "SMTP_PASS",
  ] as const;

  const faltantes = variablesObligatorias.filter(
    (variable) => !process.env[variable]
  );

  if (faltantes.length > 0) {
    throw new Error(`Faltan variables SMTP en .env: ${faltantes.join(", ")}`);
  }
};

/**
 * Obtiene el remitente.
 */
const obtenerRemitente = (): string => {
  return (
    process.env.SMTP_FROM ||
    `"Consorcia Administración" <${process.env.SMTP_USER}>`
  );
};

/* ============================================================
 * CONFIGURACIÓN SMTP
 * ============================================================ */

let transporterInstance: Transporter | null = null;

const getTransporter = (): Transporter => {
  if (!transporterInstance) {
    validarConfiguracionSmtp();

    const smtpPort: number = Number(process.env.SMTP_PORT || 587);

    transporterInstance = nodemailer.createTransport({
      host: process.env.SMTP_HOST,
      port: smtpPort,
      secure: process.env.SMTP_SECURE
        ? parseBoolean(process.env.SMTP_SECURE)
        : smtpPort === 465,
      auth: {
        user: process.env.SMTP_USER,
        pass: process.env.SMTP_PASS,
      },
      connectionTimeout: 10_000,
      greetingTimeout: 10_000,
      socketTimeout: 15_000,
      debug: false,
      logger: false,
    });
  }
  return transporterInstance;
};

/* ============================================================
 * TEMPLATE: INVITACIÓN (activación de cuenta)
 * ============================================================ */

const generarHtmlInvitacion = ({
  nombreUsuario,
  urlActivacion,
}: TemplateInvitacionParams): string => {
  const nombreSeguro = escaparHtml(nombreUsuario || "Usuario");
  const urlSegura = escaparHtml(urlActivacion);
  const year = new Date().getFullYear();

  return [
    '<!doctype html>',
    '<html lang="es">',
    '<head>',
    '<meta charset="UTF-8" />',
    '<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
    '<title>Activación de cuenta - Consorcia</title>',
    '</head>',
    '<body style="margin:0; padding:0; background-color:#f8fafc; font-family:Arial, Helvetica, sans-serif;">',
    '<div style="max-width:600px; margin:0 auto; padding:24px;">',
    '<div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:16px; overflow:hidden;">',

    // Header oscuro con branding
    '<div style="background:#0b132b; padding:28px 24px; text-align:center;">',
    '<div style="font-size:24px; font-weight:800; letter-spacing:2px; color:#ffffff;">',
    'CONSOR<span style="color:#fca311;">CIA</span>',
    '</div>',
    '<p style="margin:8px 0 0; color:#94a3b8; font-size:12px; letter-spacing:1px; text-transform:uppercase;">',
    'Gestión Inteligente',
    '</p>',
    '</div>',

    // Cuerpo del mensaje
    '<div style="padding:28px 24px;">',
    '<h2 style="margin:0 0 16px; color:#0f172a; font-size:22px; text-align:center;">',
    '¡Te damos la bienvenida a Consorcia!',
    '</h2>',
    `<p style="color:#475569; font-size:16px; margin:0 0 14px;">Hola <strong>${nombreSeguro}</strong>,</p>`,
    '<p style="color:#475569; font-size:14px; line-height:1.6; margin:0;">',
    'La administración ha creado tu cuenta. Para ingresar a la aplicación y configurar tu contraseña de acceso, hacé clic en el siguiente botón:',
    '</p>',

    // Botón CTA (CORREGIDO)
    '<div style="text-align:center; margin:32px 0;">',
    `<a href="${urlSegura}" target="_blank" rel="noopener noreferrer" style="display:inline-block; background-color:#1e6f65; color:#ffffff; padding:13px 28px; border-radius:12px; text-decoration:none; font-weight:bold; font-size:14px;">Activar mi cuenta</a>`,
    '</div>',

    // Aviso de expiración
    '<div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:14px;">',
    '<p style="color:#64748b; font-size:12px; line-height:1.5; margin:0; text-align:center;">',
    'Este enlace es de un solo uso y expirará en <strong>24 horas</strong>.',
    '</p>',
    '</div>',

    // Fallback con URL en texto plano
    '<p style="color:#94a3b8; font-size:12px; line-height:1.5; text-align:center; margin:20px 0 0;">',
    'Si el botón no funciona, copiá y pegá este enlace en tu navegador:',
    '<br />',
    `<span style="color:#0f172a; word-break:break-all;">${urlSegura}</span>`,
    '</p>',
    '</div>',

    // Footer
    '<div style="background:#f8fafc; padding:16px 24px; border-top:1px solid #e2e8f0; text-align:center;">',
    '<p style="margin:0; color:#94a3b8; font-size:11px;">',
    `© ${year} CONSORCIA • by ARTHEMYSA`,
    '</p>',
    '</div>',

    '</div>',
    '</div>',
    '</body>',
    '</html>',
  ].join("\n");
};

const generarTextoInvitacion = ({
  nombreUsuario,
  urlActivacion,
}: TemplateInvitacionParams): string => {
  return [
    `Hola ${nombreUsuario || "Usuario"},`,
    "",
    "La administración ha creado tu cuenta en Consorcia.",
    "",
    "Para activar tu cuenta y configurar tu contraseña, ingresá al siguiente enlace:",
    "",
    urlActivacion,
    "",
    "Este enlace es de un solo uso y expirará en 24 horas.",
    "",
    "Si no solicitaste esta invitación, podés ignorar este mensaje.",
    "",
    "CONSORCIA",
    "Gestión Inteligente",
  ].join("\n");
};

/* ============================================================
 * TEMPLATE: RESET DE CONTRASEÑA
 * ============================================================ */

const generarHtmlReset = ({
  nombreUsuario,
  urlReset,
}: TemplateResetParams): string => {
  const nombreSeguro = escaparHtml(nombreUsuario || "Usuario");
  const urlSegura = escaparHtml(urlReset);
  const year = new Date().getFullYear();

  return [
    '<!doctype html>',
    '<html lang="es">',
    '<head>',
    '<meta charset="UTF-8" />',
    '<meta name="viewport" content="width=device-width, initial-scale=1.0" />',
    '<title>Restablecer contraseña - Consorcia</title>',
    '</head>',
    '<body style="margin:0; padding:0; background-color:#f8fafc; font-family:Arial, Helvetica, sans-serif;">',
    '<div style="max-width:600px; margin:0 auto; padding:24px;">',
    '<div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:16px; overflow:hidden;">',

    // Header oscuro con branding
    '<div style="background:#0b132b; padding:28px 24px; text-align:center;">',
    '<div style="font-size:24px; font-weight:800; letter-spacing:2px; color:#ffffff;">',
    'CONSOR<span style="color:#fca311;">CIA</span>',
    '</div>',
    '<p style="margin:8px 0 0; color:#94a3b8; font-size:12px; letter-spacing:1px; text-transform:uppercase;">',
    'Gestión Inteligente',
    '</p>',
    '</div>',

    // Cuerpo del mensaje
    '<div style="padding:28px 24px;">',
    '<h2 style="margin:0 0 16px; color:#0f172a; font-size:22px; text-align:center;">',
    'Restablecer contraseña',
    '</h2>',
    `<p style="color:#475569; font-size:16px; margin:0 0 14px;">Hola <strong>${nombreSeguro}</strong>,</p>`,
    '<p style="color:#475569; font-size:14px; line-height:1.6; margin:0;">',
    'Recibimos una solicitud para restablecer la contraseña de tu cuenta. Para continuar, hacé clic en el siguiente botón:',
    '</p>',

    // Botón CTA (CORREGIDO)
    '<div style="text-align:center; margin:32px 0;">',
    `<a href="${urlSegura}" target="_blank" rel="noopener noreferrer" style="display:inline-block; background-color:#1e6f65; color:#ffffff; padding:13px 28px; border-radius:12px; text-decoration:none; font-weight:bold; font-size:14px;">Restablecer contraseña</a>`,
    '</div>',

    // Aviso de expiración
    '<div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:14px;">',
    '<p style="color:#64748b; font-size:12px; line-height:1.5; margin:0; text-align:center;">',
    'Este enlace es de un solo uso y expirará en <strong>1 hora</strong>.',
    '</p>',
    '</div>',

    // Aviso de seguridad
    '<div style="margin-top:20px; padding:14px; background:#fef3c7; border:1px solid #fde68a; border-radius:12px;">',
    '<p style="color:#92400e; font-size:12px; line-height:1.5; margin:0;">',
    '<strong>¿No solicitaste este cambio?</strong> Podés ignorar este mensaje. Tu contraseña actual seguirá activa. Si sospechás de actividad inusual, contactá a la administración.',
    '</p>',
    '</div>',

    // Fallback con URL en texto plano
    '<p style="color:#94a3b8; font-size:12px; line-height:1.5; text-align:center; margin:20px 0 0;">',
    'Si el botón no funciona, copiá y pegá este enlace en tu navegador:',
    '<br />',
    `<span style="color:#0f172a; word-break:break-all;">${urlSegura}</span>`,
    '</p>',
    '</div>',

    // Footer
    '<div style="background:#f8fafc; padding:16px 24px; border-top:1px solid #e2e8f0; text-align:center;">',
    '<p style="margin:0; color:#94a3b8; font-size:11px;">',
    `© ${year} CONSORCIA • by ARTHEMYSA`,
    '</p>',
    '</div>',

    '</div>',
    '</div>',
    '</body>',
    '</html>',
  ].join("\n");
};

const generarTextoReset = ({
  nombreUsuario,
  urlReset,
}: TemplateResetParams): string => {
  return [
    `Hola ${nombreUsuario || "Usuario"},`,
    "",
    "Recibimos una solicitud para restablecer la contraseña de tu cuenta en Consorcia.",
    "",
    "Para continuar, ingresá al siguiente enlace:",
    "",
    urlReset,
    "",
    "Este enlace es de un solo uso y expirará en 1 hora.",
    "",
    "Si no solicitaste este cambio, podés ignorar este mensaje. Tu contraseña actual seguirá activa.",
    "",
    "CONSORCIA",
    "Gestión Inteligente",
  ].join("\n");
};

/* ============================================================
 * SERVICIO: ENVIAR INVITACIÓN
 * ============================================================ */

export const enviarMailInvitacion = async (
  emailDestino: string,
  nombreUsuario: string | undefined,
  urlActivacion: string
): Promise<boolean> => {
  try {
    if (!emailDestino) {
      throw new Error("No se indicó el destinatario del correo.");
    }

    if (!urlActivacion) {
      throw new Error("No se indicó la URL de activación.");
    }

    const htmlContent = generarHtmlInvitacion({
      nombreUsuario,
      urlActivacion,
    });

    const textContent = generarTextoInvitacion({
      nombreUsuario,
      urlActivacion,
    });

    const mailOptions: SendMailOptions = {
      from: obtenerRemitente(),
      to: emailDestino,
      subject: "Activación de cuenta - Consorcia",
      html: htmlContent,
      text: textContent,
    };

    const info: SentMessageInfo = await getTransporter().sendMail(mailOptions);

    console.log(
      `Mail enviado con éxito a ${emailDestino} (ID: ${info.messageId})`
    );

    return true;
  } catch (error) {
    console.error("Error enviando el mail de invitación:", error);
    throw new Error("No se pudo enviar el correo de activación.");
  }
};

/* ============================================================
 * SERVICIO: ENVIAR RESET DE CONTRASEÑA
 * ============================================================ */

interface EnviarResetParams {
  email: string;
  name?: string;
  resetLink: string;
}

export const enviarEmailResetPassword = async ({
  email,
  name,
  resetLink,
}: EnviarResetParams): Promise<boolean> => {
  try {
    if (!email) {
      throw new Error("No se indicó el destinatario del correo.");
    }

    if (!resetLink) {
      throw new Error("No se indicó la URL de reset.");
    }

    const htmlContent = generarHtmlReset({
      nombreUsuario: name,
      urlReset: resetLink,
    });

    const textContent = generarTextoReset({
      nombreUsuario: name,
      urlReset: resetLink,
    });

    const mailOptions: SendMailOptions = {
      from: obtenerRemitente(),
      to: email,
      subject: "Restablecer contraseña - Consorcia",
      html: htmlContent,
      text: textContent,
    };

    const info: SentMessageInfo = await getTransporter().sendMail(mailOptions);

    console.log(
      `Mail de reset enviado con éxito a ${email} (ID: ${info.messageId})`
    );

    return true;
  } catch (error) {
    console.error("Error enviando el mail de reset de contraseña:", error);
    return false; // fire-and-forget: no propagamos el error
  }
};